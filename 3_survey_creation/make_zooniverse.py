"""
Module for creating Zooniverse subject sets from LLM response database.

This module provides functionality to convert database records of LLM responses
into Zooniverse-compatible subject sets for crowdsourced evaluation.
"""
import csv
import logging
import os
...
def make_zooniverse_subject_set_per_db(db_name: str, root_prompt_dir: str | Path, zooniverse_output_dir: str|Path, poppler_path: str = None, skip_prompt_ids: List[int] = None):
    """
        Create Zooniverse subject set from database with all responses in one manifest.

        Args:
            db_name: Path to SQLite database file
            root_prompt_dir: Directory containing prompt files
            zooniverse_output_dir: Output directory for Zooniverse files
            poppler_path: Path to Poppler utilities (optional)
            skip_prompt_ids: List of prompt IDs to skip (optional)
        """
    # Validate parameters
    if not poppler_path:
        dotenv.load_dotenv()
    ...
    logging.info('root prompt dir = %s found and is a directory',root_prompt_dir)
    logging.info('Start making zooniverse files for %s', root_prompt_dir)
    logging.info('Initiating connection to db %s',db_name)
    ...
    logging.info('Loaded % response records from database %',len(responses_list),db_name)
    ...
    for response_info in responses_list:
        ...
        rows_in_manifest.append(
            [response_id, prompt_id, prompt_image_filename, response_filename1, response_filename2, response_filename3, response_filename4, response_filename5, model_name, temperature, execution_date, llm_provider,
             prompt_run_by])
    with open(zooniverse_output_dir / 'manifest.csv', 'w', newline='', encoding='utf-8') as fw:
...
def make_zooniverse_subject_set_per_prompt(db_name: str, root_prompt_dir: str | Path, zooniverse_output_dir: str | Path,
    ...
    for prompt_id in rows_in_manifest:
        with open(zooniverse_output_dir / f'prompt_{prompt_id}' / 'manifest.csv', 'w', newline='', encoding='utf-8') as fw:
            csv_writer = csv.writer(fw, delimiter=',')
...
if __name__ == '__main__':
    ...
    file_handler = logging.FileHandler(
        f'make_zooniverse_{formatted_date}.log', mode='w', encoding='utf-8'
    )
    ...
    POPPLER_PATH = os.getenv('POPPLER_PATH')
    SKIP_PROMPT_IDS = [
        17,  # This prompt is too long to render in one page, therefore not
        # including, as confusing to a reviewer
    ]
    make_zooniverse_subject_set_per_prompt(
        r"../records.db",
        root_prompt_dir=INPUT_ROOT_DIR,
        zooniverse_output_dir=OUTPUT_DIR,
        skip_prompt_ids=[17]
    )
