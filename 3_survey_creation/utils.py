...
def save_text_as_img_markdown(md_text:str,output_path:str|Path,font_family:str="dejavusans",font_size:int=16,poppler_path='poppler/poppler-24.08.0/Library/bin',font_root_dir:str=None) -> List[str]:

    output_path = Path(output_path) if not isinstance(output_path,Path) else output_path
    if not font_root_dir:
        dotenv.load_dotenv()
        font_root_dir = os.getenv('FONT_ROOT_DIR')
        assert isinstance(font_root_dir,str), "Please set the FONT_ROOT_DIR environment variable"
    font_root_dir = Path(font_root_dir) if font_root_dir else None
    ...
    try:
        ...
        pdf.add_page()
        pdf.add_font("dejavusans", fname=str(font_root_dir / r"DejaVuSans.ttf"))
        pdf.add_font("dejavusans",style="B",
                     fname=str(font_root_dir / 'DejaVuSans-Bold.ttf'))
        pdf.add_font("dejavusans", style="I",
                     fname=str(font_root_dir / 'DejaVuSans-Oblique.ttf'))
        pdf.add_font("dejavusans", style="BI",
                     fname=str(font_root_dir / 'DejaVuSans-BoldOblique.ttf'))
